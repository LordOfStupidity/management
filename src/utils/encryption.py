...
def encrypt_obj(obj):
    ...
    for key, value in fields.items():
        ...
        if is_encrypted(encrypted_obj, key):
            try:
                real_value = type(object.__getattribute__(
                    type(encrypted_obj), key))(value)
            except ValueError:
                real_value = value
...
def decrypt_obj(obj):
    ...
    for key, value in fields.items():
        ...
        if is_encrypted(decrypted_obj, key) and value:
            setattr(decrypted_obj, key, from_bytes(
                _fernet.decrypt(value), getattr(type(decrypted_obj)(), key)))
        elif not value:
