...
"""
    ...
    id = Id(), the database object id. Always put one Id.
            ...
            if isinstance(value, property):
                ...
                try:
                    if isinstance(sub_object, list):
                        sub_dict = [obj.get_complete_dict()
        ...
        for key, value in {**arg_dict, **kwargs}.items():
            if key in vars(self):
                setattr(self, key, type(
                    object.__getattribute__(type(self), key))(value))
...
class Database:
    ...
    def __init__(self, user, password, db_name, encryption_key, logger):
        encryption.init(encryption_key)
        self.user = user
        self.password = password
        self.db_name = db_name
        self.logger = logger
        self.logger.info(f"Connecting to the database {db_name}...")
        try:
            ...
            self.db.close()
            self.db = MySQLdb.connect(user=user, passwd=password, db=db_name)
        self.logger.info(f"Connection to the database {db_name} established.")
    ...
    def reconnect(self):
        self.logger.info(f"Reconnecting to the database {self.db_name}...")
        self.db = MySQLdb.connect(
            user=self.user, passwd=self.password, db=self.db_name)
        self.logger.info(
            f"Reconnection to the database {self.db_name} successful.")
...
class Session:
    """
        ...
        arguments:
            ...
            encryption_key: encryption key.
                            FERNET_KEY is the encryption key. You can create one with
                            Fernet.generate_key(), but keep in mind to alwats use the sma eone.
    ...
    def __init__(self, user, password, db_name, encryption_key, logger):
        self.db = Database(user, password, db_name, encryption_key, logger)
    ...
    def create_table(self, obj):
        query = f"CREATE TABLE {obj.__tablename__} ("
        has_id = False
        for key, value in obj.get_table_dict().items():
            if key == "id":
                has_id = True
                query += f"{key} MEDIUMINT NOT NULL AUTO_INCREMENT, "
            elif isinstance(getattr(obj(), key), encryption.Id):
                query += f"{key} MEDIUMINT, "
            else:
                query += f"{key} BLOB, "
        if has_id:
        ...
        def update_table(self, obj):
            query = f"DESC {obj.__tablename__};"
            cursor = self.db.cursor()
            cursor.execute(query)
            ...
            for column_to_add in columns_to_add:
                query = f"ALTER TABLE {obj.__tablename__} ADD COLUMN {column_to_add}"
                if isinstance(getattr(obj(), column_to_add), encryption.Id):
                    query += " MEDIUMINT"
                else:
                    query += " BLOB"
                query += ";"
            ...
            for column_to_delete in columns_to_delete:
                query = f"ALTER TBALE {obj.__tablename__} DROP COLUMN {column_to_delete};"
                cursor = self.db.cursor()
                cursor.execute(query)
                self.db.commit()
        ...
        def query(self, obj):
            """
                Let's you perform operations with the table.
            """
            return Query(self, obj)

        def add(self, obj):
            """
                Adds a table object to the session.

                obj now automatically contains the id generated by the database.
                Also, this method returns the object in case you want to chain methods.

                Table objects supports **kwargs
            """
            obj._session = self
            ...
            encrypted_obj = encryption.encrypt_obj(obj)
            query = f"INSERT INTO {obj.__tablename__} ("
            all_values = []
        ...
        def update(self, obj):
            """
                Updates the data of the table object in session.
            ...
            if "updated_at" in obj.__dict__:
                obj.updated_at = int(datetime.datetime.utcnow().timestamp())
            encrypted_obj = encryption.encrypt_obj(obj)
            query = f"UPDATE {obj.__tablename__} SET "
            all_values = []
            obj_id = -1
            for key, value in encrypted_obj.get_table_dict().items():
                if key == "id":
        ...
        def delete(self, obj):
            """
                obj is not in the database anymore.
                This method returns the object.
                
                But You can still use it, just remember it doesn't have an id.
            """
...
class Query:
    ...
    def __init__(self, session, obj):
        ...
        self.obj = obj
        self.query = f"SELECT * FROM {obj.__tablename__}"
        self.all_values = []
    ...
    def first(self):
        ...
        if not result:
            return None
        return encryption.decrypt_obj(self.obj(self.session, result))
    ...
    def all(self):
        ...
        for result in results:
            to_return.append(encryption.decrypt_obj(
                self.obj(self.session, result)))
        return to_return
    ...
    def where(self, *args, **kwargs):
        ...
        for key, value in arguments.items():
            ...
            if isinstance(getattr(self.obj(), key), bytes):
                self.all_values.append(encryption.hash_value(value))
            else:
...
def delete(db, to_delete):
    ...
    for obj in to_delete:
        ...
        if "id" not in dic:
            return
        query = "DELETE FROM " + obj.__tablename__ + \
            " WHERE id = " + str(dic["id"]) + ";"
        cursor = db.cursor()
        cursor.execute(query)
        db.commit()
